  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        <h3 className="text-heading-4 mb-5">Colors</h3>
      ...
      {/* Drivetrain */}
      <div className="rounded-xl border border-border bg-card p-6">
        <h3 className="text-heading-4 mb-5">Drivetrain</h3>
      ...
      {/* Engine (optional, more relevant for sale) */}
      <div className="rounded-xl border border-border bg-card p-6">
        <h3 className="text-heading-4 mb-5">Engine</h3>
        <div className="grid grid-cols-3 gap-6">
          <div className="space-y-1.5">
      ...
      {/* Features */}
      <div className="rounded-xl border border-border bg-card p-6">
        <h3 className="text-heading-4 mb-5">Features</h3>
        ...
        {data.features.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-3">
            {data.features.map((f) => (
              <span key={f} className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-3 py-1 text-caption text-primary">
                {f}
                <button type="button" onClick={() => removeFeature(f)} className="rounded-full p-0.5 hover:bg-primary/20 transition-colors">
      ...
      <div className="flex justify-end pt-2">
        <Button size="lg" onClick={next}>Continue to Photos</Button>
      </div>
    </div>
  );
}
